...
| Type at cursor | **paste** (Qt clipboard + pynput, X11) | **clipboard-paste default** (Cmd+V) | **clipboard-paste default** (Ctrl+V) |
...
| Clipboard | Qt clipboard manager (preserve + history) | pyperclip (pbcopy) | pyperclip (built-in) |
...
- **clipboard** — pyperclip: built-in on Win, pbcopy on mac; Linux uses Qt's QClipboard (paste injection + history)
...
- Manual hotkey → record → transcribe → paste (Qt clipboard + pynput) identical to pre-port.
